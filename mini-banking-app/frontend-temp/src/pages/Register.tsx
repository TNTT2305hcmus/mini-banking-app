...
import { Lock, Shield, Key, CheckCircle, ArrowLeft, RefreshCw, Mail, UserRound } from "lucide-react"
...
function StepOTP({ email, onNext, onBack }: { email: string; onNext: () => void; onBack: () => void }) {
  ...
  const verify = (code: string) => {
    // demo: "000000" = sai OTP, anything else = đúng
    if (code === "000000") {
      setError(true)
      setTimeout(() => { setDigits(Array(6).fill("")); setError(false); refs.current[0]?.focus() }, 1000)
    } else {
  ...
  return (
    <>
      ...
      <div className="flex gap-2 justify-center mb-2" onPaste={handlePaste}>
        {digits.map((d, i) => (
          <input
            ...
            className={`w-11 h-13 text-center text-xl font-mono bg-background border-2 rounded-xl text-foreground focus:outline-none transition-all ${
              error
      ...
      {error && (
        <p className="text-center text-xs text-red-400 mb-2">Mã OTP không chính xác</p>
      )}

      <div className="flex items-center justify-between mt-4 text-xs text-muted-foreground">
        <span className="font-mono">{fmt(countdown)}</span>
        {countdown === 0
          ? <button onClick={() => setCountdown(300)} className="text-blue-400 hover:text-blue-300 transition-colors">Gửi lại mã</button>
          : <span className="text-muted-foreground/50">Hết hạn sau {fmt(countdown)}</span>
        }
      </div>

      <div className="mt-5 bg-background border border-border rounded-lg p-3">
        <p className="text-xs text-muted-foreground/60 font-mono">Demo: nhập <span className="text-red-400/70">000000</span> để xem lỗi · bất kỳ mã khác để tiếp tục</p>
      </div>
    </>
  )
}
...
// ─── Step 4: PIN ──────────────────────────────────────────────────────────────
function StepPIN({ onDone }: { onDone: () => void }) {
  ...
  return (
    <>
      ...
      <div className="text-center mb-2">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {subStep === "set"
            ? "Mã PIN 6 chữ số bảo vệ tài khoản của bạn"
      ...
      {subStep === "set" && (
        <p className="text-center text-xs text-muted-foreground/50 mt-4 font-mono">Không chia sẻ mã PIN với bất kỳ ai</p>
      )}
    </>
  )
}
...
export default function Register() {
  ...
  return (
    <AuthShell>
      ...
      {step === 3 && (
        <StepUsername onNext={() => setStep(4)} onBack={() => setStep(2)} />
      )}
      {step === 4 && (
        <StepPIN onDone={() => navigate("/login")} />
      )}
    </AuthShell>
  )
}
