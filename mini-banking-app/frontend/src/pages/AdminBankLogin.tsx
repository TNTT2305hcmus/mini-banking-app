...
export default function AdminBankLogin() {
  ...
  useEffect(() => {
    let active = true
    Promise.all([isEnrolled(), getStoredClientProfile()])
      .then(([hasEnrollment, profile]) => {
        if (!active) return
        setEnrolled(hasEnrollment)
        setFullName(profile?.fullName ?? "")
      })
      .catch(() => active && setError("Không thể đọc chứng chỉ Admin trên thiết bị này"))
  ...
  const verifyPin = async (candidate: string) => {
    ...
    try {
      ...
      navigate("/admin-bank", { replace: true })
    } catch (err) {
        ...
        <div className="bg-card border border-border rounded-2xl p-7 shadow-xl shadow-black/40">
          {!ready ? (
            ...
            <div className="py-6 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-2 mb-5">Không tìm thấy private key và chứng chỉ Bank Admin trong IndexedDB.</p>
              <Link to="/admin-bank/activate" className="inline-flex bg-blue-600 hover:bg-blue-500 text-white rounded-xl px-5 py-2.5 text-sm font-semibold transition-colors">
                Kích hoạt thiết bị
              </Link>
            </div>
          ) : (
          ...
          <div className="mt-5 pt-4 border-t border-border text-center">
            <p className="text-xs text-muted-foreground">
              ...
              <Link to="/admin-bank/activate" className="text-blue-400 hover:text-blue-300 transition-colors">Kích hoạt</Link>
            </p>
          </div>
        </div>
      </div>
