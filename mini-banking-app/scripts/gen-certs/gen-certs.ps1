...
.DESCRIPTION
  Creates one internal transport CA and signs server certificates for KDC and Bank.
  CA service gRPC certs are generated by ca-service/scripts/provision_ca_dev.go.
  The CA service gRPC server cert is signed by the CA service Root CA itself, so
  this script only distributes that public Root CA as the CA-service trust anchor.
...
$CaServiceCa = Join-Path $Root "ca-service\certs\grpc\ca-server-ca.crt"
...
if (-not (Test-Path $CaServiceCa)) {
    throw @"
Missing CA service Root CA trust anchor: $CaServiceCa
Run this first:
  cd "$Root\ca-service"
...
function Copy-CaServiceCa([string]$Dest) {
    New-Item -ItemType Directory -Force -Path (Split-Path $Dest) | Out-Null
    Copy-Item $CaServiceCa $Dest -Force
    Write-Host "    -> $Dest"
}
function Write-GatewayBundle([string]$Dest) {
    New-Item -ItemType Directory -Force -Path (Split-Path $Dest) | Out-Null
    $caServicePem = Get-Content -Raw -LiteralPath $CaServiceCa
    $stackPem = Get-Content -Raw -LiteralPath $CaCrt
    Set-Content -Path $Dest -Value ($caServicePem.TrimEnd() + "`n" + $stackPem.TrimEnd() + "`n") -Encoding ascii
    Write-Host "    -> $Dest"
}
Write-GatewayBundle (Join-Path $Root "api-gateway\certs\grpc-ca.crt")
Copy-CaCert (Join-Path $Root "kdc-service\certs\grpc-ca.crt")
Copy-CaServiceCa (Join-Path $Root "kdc-service\certs\grpc\ca-server-ca.crt")
Copy-CaServiceCa (Join-Path $Root "banking-service\certs\grpc-ca.crt")
