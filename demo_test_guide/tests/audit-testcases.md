...
| --- | ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | --------- | ----- | ---- |
| 1   | OTP request                                                            | CA `ra_otp_requested`, actor `ra:otp`                                                                                      | `GET /v1/admin-ca/audit?action=ra_otp_requested`                  | PASS | Thuận | Verified OTP generation logged in CA DB |
| 2   | OTP verify đúng                                                        | CA `ra_otp_verified` (metadata.owner_id)                                                                                   | `GET /v1/admin-ca/audit?action=ra_otp_verified`                   | PASS | Thuận | Verified in log after OTP verification |
| 3   | OTP verify sai/hết lượt                                                | CA `ra_otp_failed` reason `otp_mismatch`/`too_many_attempts`, severity warning                                             | `GET /v1/admin-ca/audit?action=ra_otp_failed`                     | PASS | Thuận | Verified invalid OTP verification rejection |
| 4   | PKI register thành công                                                | CA `ra_registration_approved` + `issued` (cùng request_id)                                                                 | `GET /v1/admin-ca/audit?request_id=<operation_id>`                | PASS | Thuận | Confirmed registration and cert issuance logs |
| 5   | Register fail (email trùng…)                                           | CA `ra_registration_rejected`                                                                                              | `GET /v1/admin-ca/audit?action=ra_registration_rejected`          | PASS | Thuận | Logged duplicate registration rejection |
| 6   | Admin CA login đúng/sai                                                | CA `admin_ca_login_success` / `admin_ca_login_failed`                                                                      | `GET /v1/admin-ca/audit?action=admin_ca_login_success`            | PASS | Thuận | Logged CA Admin logins |
| 7   | Mở detail cert                                                         | CA `looked_up`, category admin_action                                                                                      | `GET /v1/admin-ca/audit?action=looked_up`                         | PASS | Thuận | Logged certification detail drawers opening |
| 8   | Revoke cert (có reason)                                                | CA `revoked` severity critical                                                                                             | `GET /v1/admin-ca/audit?action=revoked`                           | PASS | Thuận | Revocation of Bob's cert logged correctly |
| 9   | AS login thành công                                                    | KDC `as_ticket_issued`                                                                                                     | `GET /v1/admin-kdc/audit?action=as_ticket_issued`                 | PASS | Thuận | KDC audited `as_ticket_issued` successfully |
| 10  | AS với cert revoked / pre-auth sai                                     | KDC `as_rejected` reason `cert_revoked`/`auth_invalid`, severity warning                                                   | `GET /v1/admin-kdc/audit?action=as_rejected`                      | PASS | Thuận | Bob's revoked AS request logged as `as_rejected` |
| 11  | TGS cấp service ticket                                                 | KDC `tgs_ticket_issued` (scope)                                                                                            | `GET /v1/admin-kdc/audit?action=tgs_ticket_issued`                | PASS | Thuận | TGS issue audited correctly |
| 12  | TGS sai scope/authenticator                                            | KDC `tgs_rejected`                                                                                                         | `GET /v1/admin-kdc/audit?action=tgs_rejected`                     | PASS | Thuận | Scope mismatch rejection logged |
| 13  | Transfer thành công                                                    | Bank `transfer_completed`                                                                                                  | `POST /v1/admin/bank/audit/query {"action":"transfer_completed"}` | PASS | Thuận | Alice's transfer audited |
| 14  | Gửi lại nonce/request                                                  | Bank `replay_detected` severity critical                                                                                   | `POST /v1/admin/bank/audit/query {"action":"replay_detected"}`    | PASS | Thuận | Duplicate execution rejected |
| 15  | Transfer/balance account không thuộc user                              | Bank `forbidden_ownership`                                                                                                 | `POST /v1/admin/bank/audit/query {"action":"forbidden_ownership"}` | PASS | Thuận | Checked unauthorized queries and transfers |
| 16  | Chữ ký payload sai                                                     | Bank `invalid_signature` severity critical                                                                                 | `POST /v1/admin/bank/audit/query {"action":"invalid_signature"}`  | PASS | Thuận | Payload signature failure audited |
| 17  | Vượt số dư                                                             | Bank `insufficient_funds`                                                                                                  | `POST /v1/admin/bank/audit/query {"action":"insufficient_funds"}` | PASS | Thuận | Logged insufficient balance error |
| 18  | Bank flow cert revoked/expired                                         | Bank `certificate_rejected` severity critical                                                                              | `POST /v1/admin/bank/audit/query {"action":"certificate_rejected"}` | PASS | Thuận | Cert rejection logged on banking actions |
| 19  | **Timeline** theo `operation_id` 1 flow register/login/transfer               | Register: `ra_otp_*→ra_registration_*→issued` (CA). Login: `as_ticket_issued` + profile TGS/AP theo cùng id. Transfer mới: `tgs_ticket_issued` + Bank event theo cùng id khi cần ticket mới. | `GET /v1/admin/audit/timeline?request_id=<operation_id>`                    | PASS | Thuận | Timelines compiled accurately |
| 20  | **Verify** khi chưa sửa gì                                             | `ok:true` mọi source                                                                                                       | `GET /v1/admin/audit/verify`                                      | PASS | Thuận | Verify ran successfully clean |
| 21  | **Verify** sau khi sửa tay 1 dòng `bank_audit_log` (đổi action/reason) | `bank.ok:false` + `broken_seq`                                                                                             | `GET /v1/admin/audit/verify`                                      | PENDING_MANUAL_DB | Thuận | Awaiting DB modification on demo stack |
| 22  | **Summary** 24h sau vài event denied                                   | `security_events`>0, `by_severity`, `top_reasons` có dữ liệu                                                               | `GET /v1/admin/audit/summary?window=24h`                          | PASS | Thuận | Summary statistics loaded correctly |
| 23  | **Anomaly** ≥5 event denied cùng danh tính                             | Xuất hiện trong `summary.anomalies`                                                                                        | `GET /v1/admin/audit/summary?window=24h`                          | PASS | Thuận | Detected excess denied anomalies |
| 24  | **Export** CSV                                                         | File CSV mở được bằng Excel, đủ cột                                                                                        | `GET /v1/admin/audit/export?format=csv`                           | PASS | Thuận | Downloaded CSV log dumps successfully |
| 25  | Query audit action rác / `limit=101` / thiếu `X-Request-ID`            | HTTP 400                                                                                                                   | curl                                                              | PASS | Thuận | Validation returns 400 |
| 26  | Query không token / sai role                                           | HTTP 401 / 403                                                                                                             | curl                                                              | PASS | Thuận | RBAC enforcement returned 401/403 |
| 27  | Tắt Postgres audit rồi chạy request chính                              | Request chính vẫn OK, log `warning: cannot insert/append audit`                                                            | service log                                                       | PASS | Thuận | Main processes do not fail if audit insert errors |

## 7.1. SOC demo checklist

| Hạng mục | Endpoint/UI | Trạng thái | Ghi chú |
|---|---|---|---|
| Login SOC bằng `security-admin` | `POST /v1/admin-sec/auth`, UI `/admin-soc` | PASS | Dùng token SOC cho KDC/timeline/verify/summary/export |
| KDC audit list | `GET /v1/admin-kdc/audit?action=as_ticket_issued` | PASS | Phụ thuộc `DATABASE_URL` của `kdc-service` trong compose |
| Timeline theo `operation_id` | `GET /v1/admin/audit/timeline?request_id=<operation_id>` | PASS | Kỳ vọng CA + KDC, thêm Bank nếu có cookie Bank Admin |
| Verify hash-chain | `GET /v1/admin/audit/verify` | PASS | Trạng thái Bank `checked:false` là hợp lệ nếu thiếu cookie |
| Summary 24h | `GET /v1/admin/audit/summary?window=24h` | PASS | Cần tạo vài denied event để summary đẹp |
| Export CSV/JSON | `GET /v1/admin/audit/export?source=all&format=csv\|json` | PASS | Lưu file làm bằng chứng rehearsal |
| Bank folded view | Các endpoint SOC + cookie `bank_admin_session` | PASS | Thiết kế chủ đích: SOC không tự đọc Bank nếu không có credential Bank Admin |
