...
Luồng đăng ký khách hàng mới gồm 2 bước nối tiếp: xác minh email bằng OTP để nhận registration token, sau đó sinh cặp khóa ở browser, tạo CSR và gửi lên CA Service để nhận X.509 client certificate do Client CA ký. Kết thúc luồng, khách hàng có wrapped private key + certificate lưu ở browser và có user record trong Bank DB.
...
- **CA Service** — verify CSR proof-of-possession, dùng Client CA ký X.509 client certificate, lưu certificate vào CA DB
- **Client CA** — Intermediate CA do Root CA ký, chuyên ký certificate cho user/client
- **Bank Service** — tạo user record trong Bank DB sau khi Client CA cấp certificate thành công
...
| Bảng | DB | Thao tác |
|---|---|---|
| `certificates` | CA DB | INSERT khi Client CA cấp cert thành công |
...
14. CA Service dùng `client-ca.key` ký X.509 client certificate từ CSR, chain về Root CA qua `client-ca.crt`, rồi lưu vào CA DB (`certificates`, `certificate_audit_log` action='issued').
15. CA Service trả `certificate_pem`, `serial_number`, `not_after_unix` và metadata issuer là Client CA.
...
| Bank Service không khả dụng sau khi Client CA đã cấp cert | 503 | Gateway revoke/mark certificate vừa cấp với reason `enrollment_failed` |
...
- User record Bank DB chỉ được tạo qua Bank Service sau khi CA Service xác nhận Client CA đã cấp cert thành công.
