...
- `blueprint/specs/02-otp-pki-registration.md`
...
3. Gửi CSR, nhận X.509 certificate từ CA.
...
| User | Bank DB `users` | Tạo sau khi enrollment thành công |
...
- `POST /v1/pki/register` với CSR hợp lệ và token hợp lệ → `201`, CA DB có cert record, Bank DB có user record.
